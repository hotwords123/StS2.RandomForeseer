...
Immediately after selection, the dispatcher updates both the detected root `Mod.manifest.version` string and the parsed `Mod.version` cache to the selected variant version. This happens before dependency validation and before the business initializer runs, so initializer-time consumers, later dependency checks, the Mod UI, telemetry, and multiplayer Mod lists all observe the version that was actually selected rather than the composite Workshop package version. Metadata already consumed before the dispatcher runs, such as duplicate-source selection and root dependency ordering, continues to come from the conservative root manifest.
