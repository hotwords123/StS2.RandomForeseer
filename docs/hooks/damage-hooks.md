...
Mirror files: `InCombat/Hooks/DamageGivenHooks.cs`, `InCombat/Hooks/DamageReceivedHooks.cs`, `InCombat/Simulation/CombatPredictionSimulator.Damage.cs`.

This document covers the implemented `BeforeDamageReceived`, `AfterDamageGiven`, and `AfterDamageReceived` mirrors plus the damage/hp/block hooks still called out as TODO comments in `ProcessDamageResults`.
...
| `RedSkull` | 红头骨 | Applies/removes Strength when owner's HP crosses threshold. | Missing. Apply/remove Power unsupported. |
| `NecroMasteryPower` | 亡灵精通 | When owner's Osty loses HP, damages enemies based on HP lost. | Missing. Damage part implementable with existing `Damage`, but owner/pet source and recursive damage risk need review. |
...
| `DemonTongue` | 恶魔之舌 | Once per turn, owner HP loss heals owner. | Marked risky when owner HP loss during owner turn occurs. Heal/shadow HP gain not currently modeled. |
...
- Still directly implementable after adding the matching post-hook dispatch: `NecroMasteryPower` damage portion in `AfterCurrentHpChanged`.
- Not implementable without architecture changes: Apply/Remove Power, summon, revive, monster move/state transitions, max HP loss, combat removal, and generalized heal hooks. This includes StS2 v0.108.0 `ConcoctPower` and `UnderworldPower` until prediction owns power application.
