...
| `ChainsOfBindingPower` | 魂缚锁链 | During owner's turn, afflicts eligible drawn cards with Bound up to a per-turn limit. | Implemented. Uses live `CardAfflictedEntry` history plus simulator `CardAfflictedHistory`; applies Bound through simulator `Afflict`. End-turn cleanup portion is in `EndTurnHooks`. |
