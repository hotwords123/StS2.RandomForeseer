...
| Damage modifier `CardPlay?` preview semantics | `OneForAllPower` | 一心化万 | Original hook is used. Generic damage forecasts still pass `null` like vanilla previews and check current modified cost. Targeted autoplay delegates can pass a constructed `CardPlay` with simulator `ResourceInfo`, so those paths use energy/stars spent/value semantics closer to real card execution. |
