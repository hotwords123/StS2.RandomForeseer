...
| Autoplay | `HowlFromBeyond`, `IAmInvincible`, `StampedePower`, `HellraiserPower` | 彼岸咆哮 / 所向无敌 / 惊逃 / 地狱狂徒 | Simulator `AutoPlay`/`AutoPlayFromDrawPile` mirrors card selection, target resolution, X/star capture, play-pile movement, supported result-pile movement, and targeted `OnPlay` delegates for `HowlFromBeyond`/`IAmInvincible`. Full card-play lifecycle is still incomplete: resource spending, `BeforeCardAutoPlayed`, `BeforeCardPlayed`/`AfterCardPlayed`, card-play history, result-pile modifier hooks, enchantment/affliction play effects, and unsupported generic `OnPlay` bodies remain omitted or risk-marked. |
