...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--branch", default="beta", choices=["public", "beta", "both"], help="Which branch to update in the workshop description files (default: beta).")
...
def ensure_clean(root: Path) -> None:
    if run_git(root, "diff", "--quiet", check=False).returncode:
        raise RuntimeError("Working tree must be clean; use --allow-dirty to override.")

    if run_git(root, "diff", "--cached", "--quiet", check=False).returncode:
        raise RuntimeError("Git index must be clean; use --allow-dirty to override.")
...
def main() -> int:
    ...
    game_version = manifest["min_game_version"]
    ...
    transformations = {
        ...
        "README.md": [
            ...
            (r"(\|\s*最低游戏版本\s*\|\s*`)[^`]+(`\s*\|)", rf"\g<1>{game_version}\g<2>", "Chinese game version"),
        ...
        "README.en.md": [
            ...
            (r"(\|\s*Minimum game version\s*\|\s*`)[^`]+(`\s*\|)", rf"\g<1>{game_version}\g<2>", "English game version"),
    ...
    if args.branch in ("public", "both"):
        transformations["workshop/description.txt"].append(
            (
                r"(\[\*]正式版：\[b]v)[^\[]+(\[/b].*?Mod 版本 \[b]v)[^\[]+(\[/b])",
                rf"\g<1>{game_version}\g<2>{target_version}\g<3>",
                "Chinese public versions",
            )
        )
        transformations["workshop/description.en.txt"].append(
            (
                r"(\[\*]public: \[b]v)[^\[]+(\[/b] \(mod version \[b]v)[^\[]+(\[/b])",
                rf"\g<1>{game_version}\g<2>{target_version}\g<3>",
                "English public versions",
            )
        )

    if args.branch in ("beta", "both"):
        transformations["workshop/description.txt"].append(
            (
                r"(\[\*]测试版：\[b]v)[^\[]+(\[/b].*?Mod 版本 \[b]v)[^\[]+(\[/b])",
                rf"\g<1>{game_version}\g<2>{target_version}\g<3>",
                "Chinese beta versions",
            )
        )
        transformations["workshop/description.en.txt"].append(
            (
