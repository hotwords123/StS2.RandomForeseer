...
1. Run `uv run scripts/prepare-release.py --mod-version X.Y.Z --game-version G.G.G --ritsu-lib-version R.R.R --commit` to update and commit the normal release files. Add `--active-game-version` once per active branch when those entries change; use `--public-game-version` and `--public-mod-version` to update the legacy public branch description.
