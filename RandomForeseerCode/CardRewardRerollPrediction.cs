...
internal static class CardRewardRerollPrediction
{
    ...
    public static IReadOnlyList<IHoverTip> GetHoverTips(CardReward reward)
    {
        ...
        var cards = PredictCards(reward);
        return cards.Select(card => (IHoverTip)new PredictionCardHoverTip(card)).ToList();
    }

    private static IReadOnlyList<CardModel> PredictCards(CardReward reward)
    {
        var player = reward.Player;
        var optionCount = OptionCountField(reward);
        ...
        var options = CloneOptions(RerollOptionsField(reward));

        var results = CreateBaseRewards(player, optionCount, options, rewardRng, rarityOdds).ToList();
        ...
        ApplyKnownAfterGeneratedModifiers(reward, results);

        return results.Select(result => result.Card).ToList();
    }

    private static CardCreationOptions CloneOptions(CardCreationOptions options)
    {
    ...
    private static void ApplyKnownAfterGeneratedModifiers(CardReward reward, List<CardCreationResult> results)
    {
        var afterGenerated = AfterGeneratedField(reward);
        ...
        foreach (var handler in afterGenerated.GetInvocationList())
        {
            ...
            {
                Entry.Logger.Warn(
                    $"Driftwood reroll prediction does not safely mirror CardReward.AfterGenerated handler {handler.Method.DeclaringType?.FullName}.{handler.Method.Name}; preview may omit that modifier.");
            }
        }
    }
    ...
    private static void WarnAboutUnsupportedModifiers(Player player, bool early)
    {
        ...
        foreach (var modifier in player.RunState.IterateHookListeners(null))
        {
            ...
            Entry.Logger.Warn($"Driftwood reroll prediction does not safely mirror {type.FullName}.{methodName}; preview may omit that modifier.");
        }
    }
